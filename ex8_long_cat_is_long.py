...
def count_words(text):
    extract_odd_symbols=[c if c not in symbol_to_remov else " " for c in text]
    text=("".join(extract_odd_symbols)).lower()     #from list to  str

    return {word:len(word) for word in text.split(" ") if word}
